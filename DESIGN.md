...
With symbol height `S`, the visible mark matches the full wordmark ink height:
from the top of the `O` to the bottom of the `p`. Align those two edges. Public
Sans at the registered weights measures 0.894 em across those bounds, with
0.161 em below the baseline. Use the largest mark that fits these bounds.
The symbol-to-wordmark gap is `0.375 S`; the stacked gap is `0.31 S`.
The product line sets at `0.62` of the wordmark cap height, with its baseline
`1.55` cap heights below the wordmark and its left edge flush with the mark.

The generator extracts the glyph bounds from the pinned font. Tests require the
symbol to match those bounds and preserve the baseline alignment.
